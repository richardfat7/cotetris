// NOTE: this bag should only be contained by root server
...
const bagOppo = (state = initState, action) => {
  switch (action.type) {
    case reducerType.SHIFT_TWICE_OPPO: {
      let sbag = state;
    ...
    case reducerType.SHIFT_NEXT_BLOCK_OPPO: {
      let sbag = state;
      sbag = sbag.shift();
      if (sbag.size < 7) {
        let newBag = List();
    ...
    case reducerType.RESET_BAG_OPPO: {
      let sbag = List();
      const len = blockType.length - 1;
      for (let i = 0; i < len; i++) {
