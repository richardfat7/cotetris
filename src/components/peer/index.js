...
export default class Peer extends React.Component {
    ...
    register(id) {
        const peer = new Peerjs(id, this.state.peerJsConfig);
    ...
    changeConfig() {
        if (this.modal4 && this.modal4.value) {
            this.setState({ peerJsConfig: JSON.parse(this.modal4.value) });
        }
    }
    ...
    render() {
        ...
        return (
            <div>
                <p>Peerjs in use</p>
                <p>config: { JSON.stringify(peerJsConfig) }</p>
                <p>myid: { id }</p>
                <p>fdid: { fd }</p>
                <p>oppid: { opp }</p>
                <p>Config:
                    <input
                        id="config"
                        type="text"
                        defaultValue={JSON.stringify(peerJsConfig)}
                        ref={(m) => { this.modal4 = m; }}
                    />
